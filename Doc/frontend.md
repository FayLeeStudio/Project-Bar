...
| 参数 | 值 | 说明 |
|---|---|---|
| 网格 W × H | 80 × 200 | 服务端持有的权威网格；客户端只显示其中一个窗口 |
| 像素格 S | 3px | 渲染时每格 = 3×3 像素 |
| 可视行数 `viewRows` | 170 行 | = 510px |
| 出口 `SPOUT_X` | {1:40, 2:15, 3:57, 4:72} | 按客观颜色槽位的固定列（见下"出口布局"） |
| `SPAWN_GAP` | 52 行 | 出沙口在堆顶上方这么多行（服务端 spawn 位置，客户端据此画水龙头） |
| 重力子步 | 2 / tick | 服务端每 tick 推进 2 次（下落更顺） |
| 镜头锚点 | 0.382（黄金分割） | 客户端让堆顶维持在视口从顶往下 0.382 处，过半才上移 |

> 服务端权威下没有客户端"冻结（`FREEZE_AT`/`BAND`）"概念——整张网格直接持久化。"沉降压缩成 1px 细条"是 Stage 3 的展示层（见 `game-design.md`）。
...
房间最多 4 人，地位完全对等。**当前实现**：出口位置按**客观颜色槽位**固定在 `SPOUT_X = {1:40, 2:15, 3:57, 4:72}`（列号，基于 `W=80`），由服务端 spawn 使用。出口在 world 的纵向位置跟随堆顶上移（`surface - SPAWN_GAP`），所以水龙头始终在堆顶上方一段、靠近视口顶。

> 早期设想的"按在线人数居中对称排列"尚未实现，留作以后的视觉优化（改了也只影响新沙从哪开始落，不动已沉积像素）。
