...
| 归档带 `bands`（Stage 3） | 每条按**真实高度** `rows` 展开 | 活动网格下方的历史层。压缩只是后台存储手段，**不做** 1px 细条特殊显示：每条 band 在世界坐标里占它真实的 `rows` 行，滚动到附近时"展开"成正常高度的彩色层，历史读起来是一座连续的塔。相机范围 `worldH = H + archiveRows`（`archiveRows = Σ band.rows`） |
| 归档查找 `archiveSeg` | `[{top,rows,bi}]`，bands 变化时重建 | 世界行 → band 的连续分段索引（seg 0 = 最新、紧贴活动网格）。每帧渲染是**窗口化**的增量行走（只碰 `viewRows` 行 + 跨段时推进指针），这就是"缓存"——拖过很长历史也只渲染视口内那些行，保持流畅 |
| 压缩时的相机 | 收到 `band` → `cameraY += rows` | 客户端做与服务端**完全相同**的确定性下移（grid 整体下移 K 行）；因为 band 按真实高度占 K 个世界行，活动地表与所有更老的 band **一律**下移 K，故单条 `cameraY += K` 就能让视图纹丝不动——无论你在跟随地表还是深在历史里。压缩在正常使用下不可见 |

> 服务端权威下没有客户端"冻结（`FREEZE_AT`/`BAND`）"概念。沉积分两段存：堆顶一段全分辨率留在活动网格里跑物理；很深的老沙由**服务端**压缩成薄层归档（`bands`，每列主色）。客户端把归档按真实高度展开渲染（有损：每列只剩主色），压缩对用户透明。机制见 `stage3-compression.md` 与 `backend.md`，叙事见 `game-design.md`。
